...
public class colaboradorRest {
	...
	private ResponseEntity<colaborador> saveColaborador(@RequestBody colaborador colaborador){
		...
		try {
			colaborador saveColaborador = colaboradorService.save(colaborador);
			return ResponseEntity.created(new URI("/colaborador/"+saveColaborador.getId())).body(saveColaborador);
